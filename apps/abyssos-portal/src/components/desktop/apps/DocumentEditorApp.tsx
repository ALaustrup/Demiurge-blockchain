    ...
    try {
      ...
      if (documentType === 'text') {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_ABYSSID_API_URL || 'http://localhost:8082'}/api/storage/upload`,
          {
            method: 'POST',
            headers: {
    ...
    return (
      <div className="h-full flex items-center justify-center p-8">
        <div className="text-center">
          <p className="text-gray-400 mb-4">Please log in with AbyssID to use the document editor</p>
        </div>
      </div>
    );
  }
