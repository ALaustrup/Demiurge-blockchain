/**
 * Abyss Explorer - Web3 Browser Application
 * 
 * A full-featured Web2+Web3 browser integrated with AbyssID and Abyss Wallet.
  ...
  if (path === 'home' || path === '') {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center p-8 text-center overflow-y-auto">
        ...
        <h2 className="text-2xl font-bold text-abyss-cyan mb-2">Abyss Explorer</h2>
...
export function AbyssExplorerApp() {
  ...
  const handleIframeError = useCallback(() => {
    setIframeError("This site can't be displayed in Abyss Explorer (likely blocking iframe embeds)");
    if (activeTab) {
      updateTab(activeTab.id, { isLoading: false });
    }
  }, [activeTab, updateTab]);
