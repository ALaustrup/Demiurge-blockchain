/**
 * AbyssID Identity Service
 * 
 * Centralized service for managing AbyssID authentication and user data synchronization
 * across all apps and services in AbyssOS.
 * 
 * This is the single source of truth for user identity on-chain.
 */

import type { AbyssIDSession } from '../abyssid/types';
import { deriveDemiurgePublicKey } from '../wallet/demiurgeWallet';
import { useWalletStore } from '../../state/walletStore';
import { useAuthStore } from '../../state/authStore';

export interface AbyssIDIdentity {
  session: AbyssIDSession;
...
class AbyssIDIdentityService {
  private identity: AbyssIDIdentity | null = null;
  private listeners: Set<(identity: AbyssIDIdentity | null) => void> = new Set();
   ...
   */
  async initialize(session: AbyssIDSession): Promise<AbyssIDIdentity> {
    // Derive Demiurge public key from AbyssID
    const demiurgePublicKey = await deriveDemiurgePublicKey(session.publicKey);
    
    const identity: AbyssIDIdentity = {
      session,
      demiurgePublicKey,
   ...
   */
  getIdentity(): AbyssIDIdentity | null {
   ...
   */
  subscribe(listener: (identity: AbyssIDIdentity | null) => void): () => void {
   ...
   */
  private notifyListeners(identity: AbyssIDIdentity | null): void {
...
// Singleton instance
export const abyssIDIdentityService = new AbyssIDIdentityService();
