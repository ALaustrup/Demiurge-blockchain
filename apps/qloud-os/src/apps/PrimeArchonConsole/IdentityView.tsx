  ...
  return (
    <div className="identity-view bg-gray-900 p-4 rounded border border-purple-500">
      ...
      <div className="identity-content space-y-4">
        <div>
          <div className="text-sm text-genesis-text-tertiary">Archon ID</div>
        ...
        <div>
          <div className="text-sm text-genesis-text-tertiary mb-2">Personality Traits</div>
          {Object.entries(identity.personalityTraits).map(([trait, strength]: [string, any]) => (
            <div key={trait} className="mb-2">
              <div className="flex justify-between mb-1">
                <span className="text-sm">{trait}</span>
                <span className="text-sm text-genesis-text-tertiary">{(strength * 100).toFixed(0)}%</span>
              </div>
