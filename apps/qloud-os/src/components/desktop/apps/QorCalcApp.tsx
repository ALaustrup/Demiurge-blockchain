  ...
  return (
    <div className="h-full w-full flex flex-col bg-genesis-glass-light/90 text-gray-100">
      <div className="px-6 py-4 border-b border-genesis-border-default/20 flex items-center justify-between">
        <div>
          <div className="text-2xl font-bold text-genesis-cipher-cyan">QOR Calc</div>
          <div className="text-sm text-genesis-text-tertiary">Scientific, programmer, graphing, and AI assist</div>
        </div>
          ...
          {(['basic', 'scientific', 'programmer', 'graph', 'ai'] as Mode[]).map((m) => (
            <button
              ...
              className={`px-3 py-1 rounded text-sm ${
                mode === m ? 'bg-abyss-cyan text-black font-semibold' : 'bg-genesis-glass-light border border-genesis-border-default/20'
              }`}
            >
              {m === 'basic' && 'Basic'}
      ...
      <div className="p-4 space-y-4">
        {mode !== 'ai' && (
          <>
            <input
              className="w-full px-3 py-2 bg-genesis-glass-light border border-genesis-border-default/30 rounded text-lg font-mono"
        ...
        {mode === 'ai' && (
          <div className="space-y-2">
            <textarea
              className="w-full min-h-[160px] px-3 py-2 bg-genesis-glass-light border border-genesis-border-default/30 rounded text-sm"
        ...
        <div className="bg-genesis-glass-light/60 border border-genesis-border-default/20 rounded-lg p-3 min-h-[120px]">
          <div className="text-xs text-genesis-text-tertiary mb-1">Result</div>
          <div className="text-lg font-mono text-genesis-cipher-cyan whitespace-pre-wrap">
            {result || '—'}
          </div>
        </div>
        ...
        <div className="bg-genesis-glass-light/60 border border-genesis-border-default/20 rounded-lg p-3">
          ...
          <div className="space-y-1 max-h-48 overflow-auto">
            ...
            {history.map((h, idx) => (
              <div key={`${h.expr}-${idx}`} className="flex items-center justify-between text-xs font-mono bg-genesis-glass-light/40 px-2 py-1 rounded">
                <span className="text-genesis-text-secondary">{h.expr}</span>
                <span className="text-genesis-cipher-cyan">{h.result}</span>
              </div>
            ))}
          </div>
        </div>
